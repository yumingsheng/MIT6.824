...
type Raft struct {
	...
	applyCh   	chan ApplyMsg // exactly once enforced by a single goroutine
	applyCond 	*sync.Cond  // wake up applier goroutine after committing new entries
	replicatorCond []*sync.Cond // wake up replicator goroutine to batch entries
	state     	NodeState

	currentTerm int
	votedFor    int
	logs        []Entry // the 1st entry is dummy contains LastSnapshotTerm, LastSnapshotIndex and nil Command
...
func (rf *Raft) readPersist(data []byte) {
	...
	// There will always be at least one dummy entry in rf.logs
...
//
// Assumption lastIncludedTerm >= rf.currentTerm
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	...
	DPrintf("{Node %v}'s state is {state %v,term %v,commitIndex %v,lastApplied %v,firstLog %v,lastLog %v} after accepting the snapshot which lastIncludedTerm is %v, lastIncludedIndex is %v",
...
func (rf *Raft) RequestVote(request *RequestVoteRequest, response *RequestVoteResponse) {
	...
	// Try to pick leader w/ latest logs
	...
	for peer := range rf.peers {
		...
		go func(peer int) {
			...
			if rf.sendRequestVote(peer, request, response) {
				rf.mu.Lock()
				defer rf.mu.Unlock()
				// rf.currentTerm > response.Term outdated response
				// rf.currentTerm < response.Term peer is the leader
...
func (rf *Raft) ChangeState(state NodeState) {
	...
	DPrintf("{Node %d} changes state from %s to %s in term %d",
...
func (rf *Raft) advanceCommitIndexForLeader() {
	...
	if newCommitIndex > rf.commitIndex {
		...
		if rf.matchLog(rf.currentTerm, newCommitIndex) {
			...
			DPrintf("{Node %d} cannot advance commitIndex from %d because the term of newCommitIndex %d is not equal to currentTerm %d",
...
// If index > rf.GetLastLog(), missing logs should be re-sent
...
// Used by replicator goroutine to judge whether a peer needs
// replicating.
...
// A dedicated applier goroutine to guarantee that each log will be
// pushed into applyCh exactly once, ensuring that service's applying
// entries and raft's committing entries can be parallel
func (rf *Raft) applier() {
	for rf.killed() == false {
		rf.mu.Lock()
		// If there is no need to apply entries, just release CPU and
		// wait other goroutine's signal if they commit new entries
		...
		// Use commitIndex rather than rf.commintIndex because 
		// rf.commitIndex may change during the Unlock() and Lock().
		// Use Max(rf.lastApplied, commitIndex) rather than commitIndex
		// directly to avoid concurrently InstallSnapshot rpc causing
		// lastApplied to rollback.
...
func (rf *Raft) replicator(peer int) {
	...
	for rf.killed() == false {
		// If there is no need to replicate entries for this peer, just
		// release CPU and wait other goroutine's signal if service adds
		// new Command.
		// If this peer needs replicating entries, this goroutine will
		// call replicateOneRound(peer) multiple times until this peer
		// catches up, and then wait.
		for !rf.needReplicating(peer) {
			rf.replicatorCond[peer].Wait()
		}
		// Maybe a pipeline mechanism is better to trade-off the memory
		// usage and catch up time.
